    ...
    return (
      <motion.div
        ...
        style={{
          ...
          boxShadow:
            cardData.selected && cardData.color === COLORS.default
              ? "0 0 15px 5px rgba(128, 128, 128, 0.4), 0 0 30px 10px rgba(128, 128, 128, 0.2)"
...
const GameStatus = memo(function cachedGameStatus({
  gameStarted,
  gameTimer,
  score,
  chancesLeft,
    ...
    <div className="w-full flex justify-between items-center mb-2">
      <div className="flex items-center gap-2">
        ...
        <span className="font-bold text-indigo-800">
          {gameStarted ? `Time: ${gameTimer}s` : "Get Ready!"}
        </span>
      </div>
...
const Maze = () => {
  ...
  const runHighlightSequence = useCallback(() => {
    if (
      ...
      currentHighlightIndex < highlightSequence.length
    ) {
